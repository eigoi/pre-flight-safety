...
const COMMON_CHECKS = [
  {
    ...
    classificationReason: "天候や風だけでは国土交通省の特定の事故・重大事件基準に直接対応しないため、通常は一般的な安全確認として扱います。周辺に建物や電力設備がある場合は、風による流されから第三者物件の損壊につながる可能性を考慮し、表示区分を引き上げることがあります。",
    mlitReference: "通常時：国土交通省の事故・重大事件区分には直接割り当てません。",
    actionCost: "MEDIUM",
  ...
  },
  {
    ...
    classificationReason: "機体や推進系の不具合は制御不能につながる可能性があります。国土交通省では機体不具合による制御不能を重大事件の対象としており、FAAのHazardousは想定結果の重大度を考えるための補助的な参考として使用しています。",
    mlitReference: "重大事件参考：機体不具合による制御不能。",
    actionCost: "LOW",
  ...
  },
  {
    ...
    mlitReference: "重大事件参考：一定の通信障害を含む機体不具合による制御不能。",
    actionCost: "LOW",
  ...
  },
  {
    ...
    faaSeverity: "MAJOR",
    consequenceClass: "GENERAL",
    classificationReason: "飛行ルートや緊急時の計画は重要な予防的確認ですが、この確認項目自体を国土交通省の特定の事故・重大事件基準には直接割り当てていません。",
...
async function runLegalScreening(lat, lon) {
  ...
  return {
    ...
    did: {
      ...
      detail: "地図上の赤い箇所の人口集中地区（令和2年国勢調査より）において飛行する際、免許もしくは飛行許可が必要になる場合があります。改めて最新の国土交通省による人口集中地区情報を確認した上で、もし飛行場所がその域内であればDIPS2.0より許可・承認申請を行ってください。",
    },
  };
}
...
function generateSafetyChecks(context = null) {
  ...
  if (context?.hasVegetation) {
    checks.push({
      ...
      faaSeverity: "MAJOR",
      consequenceClass: "GENERAL",
      classificationReason: "樹木や枝への接触は墜落につながる可能性がありますが、負傷や第三者の人工物の損壊など明確な結果が生じない限り、国土交通省の単一の事故・重大事件基準には直接対応しません。そのため本プロトタイプでは一般安全確認として表示します。",
  ...
  checks.push({
    ...
    faaSeverity: "MAJOR",
    consequenceClass: "GENERAL",
    classificationReason: "これは単一の故障状態や報告対象事象ではなく、広い範囲を対象とする予防的確認のため、国土交通省の事故・重大事件区分には直接割り当てません。",
    mlitReference: "一般安全確認：国土交通省の単一の報告対象区分には直接割り当てません。",
    actionCost: "LOW",
